<div>
    <div class="page-meta">
        <nav class="breadcrumb-style-one" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a>Layanan / Service</a></li>
            </ol>
        </nav>
    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td><img src="/assets/service/{{ $service->image }} " alt="" style="max-width: 80px" class="img-fluid mb-1"></td>
                                <td>{{ $service->name }}</td>
                                <td>Rp. {{ number_format($service->price, 0, ',', '.') }}</td>
                                <td>{{ $service->hourse }} Jam</td>
                                <td class="text-center">
                                    <div class="action-btns">
                                        <a href="{{ route('serviceEdit', $service->uuid) }}" class="action-btn btn-edit bs-tooltip me-2" data-toggle="tooltip" data-placement="top" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                                        </a>
                                        <a wire:confirm="Apakah anda yakin ingin menghapus?" wire:click="deleteService({{ $service->id }})" class="action-btn btn-delete bs-tooltip" data-toggle="tooltip" data-placement="top" title="Delete">
                            ...
                            @empty
                            <tr>
                                <td colspan="10">Tidak ada list layanan mobil</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                    @if (count($services) >= $limitData)
